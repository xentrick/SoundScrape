...
def process_soundcloud(vargs):
    ...
    try:
        if one_track:
            ...
            resolved = client.get('/users/' + userId + '/favorites', linked_partitioning=1)
            next_href = False
        ...
        if not hasattr(resolved, 'kind'):
            ...
            if resolved.kind == 'artist':
                ...
                tracks = client.get('/groups/' + group_id + '/tracks') 
            else:
                artist = resolved
                artist_id = str(artist.id)
                tracks = client.get('/users/' + artist_id + '/tracks') 
...
def get_soundcloud_api2_data(artist_id):
    ...
    v2_url = "https://api-v2.soundcloud.com/stream/users/%s?client_id=%s&app_version=%s" % (
